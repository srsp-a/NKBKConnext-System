...
/** อ่านจาก process.env ตอนเรียก (รองรับ Firebase Secret Manager) */
function getMonitorSystemUploadSecret() {
  return (process.env.MONITOR_SYSTEM_UPLOAD_SECRET || '').trim();
}
function getMonitorSystemPublicReadKey() {
  return (process.env.MONITOR_SYSTEM_PUBLIC_READ_KEY || '').trim();
}
  ...
  try {
    ...
    if (!admin.apps.length) {
      if (p && fsSync.existsSync(p)) {
        ...
        console.log('[Monitor API] firebase-admin พร้อม — Application Default Credentials (Cloud Functions)');
      } else {
...
app.post('/api/monitor-system-snapshot', (req, res) => {
  ...
  if (hdr !== getMonitorSystemUploadSecret()) {
 ...
 * ค่า = MONITOR_PROGRAM_MANUAL_SYNC_KEY (ถ้าตั้งใน .env) มิฉะนั้น = getMonitorSystemUploadSecret() (เดียวกับแอป)
 *
 * ลงทะเบียน 2 path: ยาว + สั้น (กรณี proxy / เวอร์ชันเก่า)
 */
async function routePostProgramsManualSync(req, res) {
  const uploadSec = (getMonitorSystemUploadSecret() || '').trim();
...
app.get('/api/monitor-system-snapshots', (req, res) => {
  if (!getMonitorSystemPublicReadKey()) {
  ...
  if (qk !== getMonitorSystemPublicReadKey() && hk !== getMonitorSystemPublicReadKey()) {
 ...
 * Header: X-Monitor-System-Secret = getMonitorSystemUploadSecret()
...
// =====================================================
// Start / Passenger / Firebase Functions
// =====================================================
/** เมื่อถูก require จาก Cloud Functions — ห้าม listen (Firebase รับ HTTP แทน) */
function shouldStartHttpServer() {
  if (process.env.SKIP_HTTP_LISTEN === '1') return false;
  if (process.env.K_SERVICE) return false;
  if (process.env.FUNCTION_TARGET) return false;
  return true;
}
...
console.log(
  ...
  __dirname,
  '| httpServer=',
  shouldStartHttpServer() ? 'on' : 'off (Functions/module)'
);
...
const usePassengerSocket = shouldStartHttpServer() && (psg || passengerEnvLikely());
