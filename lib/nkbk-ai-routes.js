...
function registerNkbkAiRoutes(app, deps) {
  ...
  async function verifyStaffAccess(req) {
    ...
    if (bearer) {
      try {
        ...
        if (adminUser) {
          return memberChatInbox.mapUserToStaff({
            uid: adminUser.uid,
            fullname: String(adminUser.name || adminUser.email || 'ผู้ดูแลระบบ').trim(),
    ...
    if (monitorToken && typeof resolveSession === 'function') {
      try {
  ...
  app.get('/api/public-cms-chat/staff-contact', async (req, res) => {
    try {
      const db = await withDb(req, res, '/api/public-cms-chat/staff-contact');
      if (!db) return;
      const code = String(req.query.code || '').trim().toLowerCase();
      const staffId = String(req.query.staffId || req.query.contact || '').trim();
      let info;
      if (code) {
        info = await staffContactAvailability.evaluateContactByShortCode(db, code);
      } else if (staffId) {
        info = await staffContactAvailability.evaluateContactAvailability(db, staffId);
      } else {
        return res.status(400).json({ ok: false, message: 'ต้องระบุ staffId หรือ code' });
      }
      if (!info.ok) return res.status(404).json({ ok: false, message: info.message || 'ไม่พบเจ้าหน้าที่' });
      if (info.staff && info.staff.id) {
        try {
          await staffContactAnalytics.recordLinkOpen(db, info.staff.id, code || info.staff.shortCode || '');
        } catch (e) {
          console.error('[staff-contact] link open:', e.message);
        }
      }
      return res.json({ ok: true, ...info });
    } catch (e) {
      console.error('[public-cms-chat] staff-contact:', e.message);
      return res.status(500).json({ ok: false, message: e.message });
    }
  });

  app.post('/api/admin/staff-contact-link/ensure', async (req, res) => {
    try {
      const adminUser = await verifyAdminAccess(req);
      if (!adminUser) return res.status(403).json({ ok: false, message: 'ไม่มีสิทธิ์แอดมิน' });
      const db = await withDb(req, res, '/api/admin/staff-contact-link/ensure');
      if (!db) return;
      const staffId = String((req.body && req.body.staffId) || '').trim();
      if (!staffId) return res.status(400).json({ ok: false, message: 'ต้องระบุ staffId' });
      const code = await staffContactLinks.ensureStaffContactShortCode(db, staffId);
      const url = staffContactLinks.publicShortUrl(code);
      return res.json({ ok: true, code, url });
    } catch (e) {
      console.error('[staff-contact-link] ensure:', e.message);
      return res.status(400).json({ ok: false, message: e.message || 'สร้างลิงก์ไม่สำเร็จ' });
    }
  });

  app.get('/api/admin/staff-contact-stats', async (req, res) => {
    try {
      const adminUser = await verifyAdminAccess(req);
      if (!adminUser) return res.status(403).json({ ok: false, message: 'ไม่มีสิทธิ์แอดมิน' });
      const db = await withDb(req, res, '/api/admin/staff-contact-stats');
      if (!db) return;
      const staffId = String(req.query.staffId || '').trim();
      const days = Number(req.query.days) || 30;
      const stats = await staffContactAnalytics.getStaffStats(db, staffId, days);
      if (!stats.ok) return res.status(400).json({ ok: false, message: 'ไม่พบข้อมูล' });
      return res.json(stats);
    } catch (e) {
      console.error('[staff-contact-stats]:', e.message);
      return res.status(500).json({ ok: false, message: e.message });
    }
  });

  app.post('/api/public-cms-chat/rating', async (req, res) => {
    try {
      const db = await withDb(req, res, '/api/public-cms-chat/rating');
      if (!db) return;
      const result = await memberChatInbox.submitContactRating(db, req.body || {});
      return res.json(result);
    } catch (e) {
      console.error('[public-cms-chat] rating:', e.message);
      return res.status(400).json({ ok: false, message: e.message || 'ส่งคะแนนไม่สำเร็จ' });
    }
  });
  ...
  app.get('/api/public-cms-chat/poll', async (req, res) => {
    try {
      ...
      const result = await memberChatInbox.pollPublicMessages(db, conversationId, since, nkbkAi);
  ...
  app.post('/api/public-cms-chat/presence', async (req, res) => {
    try {
      const db = await withDb(req, res, '/api/public-cms-chat/presence');
      if (!db) return;
      const body = req.body && typeof req.body === 'object' ? req.body : {};
      const conversationId = String(body.conversationId || '').trim();
      const result = await memberChatInbox.touchVisitorPresence(db, conversationId, body);
      return res.json(result);
    } catch (e) {
      console.error('[public-cms-chat] presence:', e.message);
      return res.status(400).json({ ok: false, message: e.message || 'presence failed' });
    }
  });

  app.post('/api/public-cms-chat/image', async (req, res) => {
    try {
      const db = await withDb(req, res, '/api/public-cms-chat/image');
      if (!db) return;
      const result = await memberChatInbox.visitorSendImage(db, req.body || {});
      return res.json({ ok: true, ...result });
    } catch (e) {
      console.error('[public-cms-chat] image:', e.message);
      return res.status(400).json({ ok: false, message: e.message || 'ส่งรูปไม่สำเร็จ' });
    }
  });
  ...
  app.post('/api/member-chat/conversations/:id/presence', async (req, res) => {
    try {
      const staff = await verifyStaffAccess(req);
      if (!staff) return res.status(403).json({ ok: false, message: 'ไม่มีสิทธิ์' });
      const db = await withDb(req, res, '/api/member-chat/conversations/:id/presence');
      if (!db) return;
      const body = req.body && typeof req.body === 'object' ? req.body : {};
      const result = await memberChatInbox.touchStaffPresence(db, staff, req.params.id, body);
      return res.json({ ok: true, ...result });
    } catch (e) {
      console.error('[member-chat] presence:', e.message);
      return res.status(400).json({ ok: false, message: e.message });
    }
  });

  app.post('/api/member-chat/conversations/:id/allow-member-images', async (req, res) => {
    try {
      const staff = await verifyStaffAccess(req);
      if (!staff) return res.status(403).json({ ok: false, message: 'ไม่มีสิทธิ์' });
      const db = await withDb(req, res, '/api/member-chat/conversations/:id/allow-member-images');
      if (!db) return;
      const body = req.body && typeof req.body === 'object' ? req.body : {};
      const allow = body.allow !== false && body.allow !== 0 && body.allow !== '0';
      const result = await memberChatInbox.setAllowMemberImages(db, staff, req.params.id, allow);
      return res.json({ ok: true, ...result });
    } catch (e) {
      console.error('[member-chat] allow-member-images:', e.message);
      return res.status(400).json({ ok: false, message: e.message });
    }
  });

  app.post('/api/member-chat/conversations/:id/image', async (req, res) => {
    try {
      const staff = await verifyStaffAccess(req);
      if (!staff) return res.status(403).json({ ok: false, message: 'ไม่มีสิทธิ์' });
      const db = await withDb(req, res, '/api/member-chat/conversations/:id/image');
      if (!db) return;
      const body = req.body && typeof req.body === 'object' ? req.body : {};
      const result = await memberChatInbox.staffSendImage(db, staff, req.params.id, body);
      return res.json(result);
    } catch (e) {
      console.error('[member-chat] staff image:', e.message);
      return res.status(400).json({ ok: false, message: e.message });
    }
  });
  ...
  app.get('/api/member-chat/staff-options', async (req, res) => {
    try {
      const staff = await verifyStaffAccess(req);
      if (!staff) return res.status(403).json({ ok: false, message: 'ไม่มีสิทธิ์' });
      const db = await withDb(req, res, '/api/member-chat/staff-options');
      if (!db) return;
      const data = await memberChatInbox.listStaffForAssign(db, staff);
      return res.json({ ok: true, ...data });
    } catch (e) {
      console.error('[member-chat] staff-options:', e.message);
      return res.status(500).json({ ok: false, message: e.message });
    }
  });
  ...
  app.get('/api/member-chat/stats', async (req, res) => {
    try {
      ...
      return res.json({
        ok: true,
        stats,
        isFullAccess: memberChatInbox.isFullInboxAccess(staff),
        canAssign: memberChatInbox.canAssign(staff)
      });
    } catch (e) {
  ...
  try {
    ...
    registerAnalyticsRoutes(app, { getDb, verifyAdminAccess });
  } catch (e) {
    console.warn('[nkbk-ai] analytics routes skipped:', e.message);
  }
}
