...
/** OpenAI key+model สำหรับงานภายใน (Meetdoc ฯลฯ) — ใช้ config/ai_chat (ตั้งค่า AI แชท) ก่อน desktop */
async function getOpenAiCredentialsForInternal(db) {
  const lineRaw = await getAiChatConfig(db);
  const chatKey = lineRaw && String(lineRaw.openaiApiKey || '').trim();
  if (chatKey) {
    return {
      openaiApiKey: chatKey,
      model: String(lineRaw.model || 'gpt-4o-mini').trim() || 'gpt-4o-mini'
    };
  }
  const desktopRaw = await getDesktopAiConfig(db);
  const desktop = normalizeDesktopConfig(desktopRaw, 'desktop_ai');
  if (desktop && String(desktop.openaiApiKey || '').trim()) {
    return {
      openaiApiKey: String(desktop.openaiApiKey).trim(),
      model: desktop.model || 'gpt-4o-mini'
    };
  }
  const fromLine = configFromAiChat(lineRaw);
  if (fromLine && String(fromLine.openaiApiKey).trim()) {
    return { openaiApiKey: String(fromLine.openaiApiKey).trim(), model: fromLine.model || 'gpt-4o-mini' };
  }
  return null;
}
...
function callOpenAIChat(apiKey, model, messages, maxTokens, chatOptions) {
  if (!apiKey || !String(apiKey).trim()) {
    return Promise.reject(new Error('OpenAI API Key not configured'));
  }
  const body = JSON.stringify(buildChatCompletionBody(model, messages, maxTokens, chatOptions));
  const timeoutMs = (chatOptions && chatOptions.timeout) || 30000;
  return new Promise((resolve, reject) => {
    const reqOptions = {
      hostname: 'api.openai.com',
      port: 443,
    ...
    req.on('error', reject);
    req.setTimeout(timeoutMs, () => req.destroy(new Error('OpenAI chat timeout')));
    req.write(body);
    req.end();
  });
}
...
function buildPublicMemberSystemPrompt(lineCfg, dataContextStr) {
  ...
  const parts = [
    ...
    'เมื่อถามแบบฟอร์ม/ดาวน์โหลดโดยยังไม่ระบุชื่อแบบฟอร์ม (เช่น กด "แบบฟอร์มดาวน์โหลด") ให้ถามกลับว่าต้องการแบบฟอร์มเรื่องอะไร และยกตัวอย่างหมวดจาก [แบบฟอร์มดาวน์โหลด] — ห้ามแนะนำหรือระบุชื่อแบบฟอร์มเฉพาะรายการ',
    'เมื่อสมาชิกระบุชื่อหรือเรื่องแบบฟอร์มชัดเจนแล้ว ให้ตอบจาก [แบบฟอร์มดาวน์โหลด] แนะนำชื่อแบบฟอร์มที่ตรงที่สุดเพียงรายการเดียว หรือดูเพิ่มเติมที่ [ดาวน์โหลด](/download) — ห้ามใส่ URL ไฟล์',
...
module.exports = {
  ...
  getOpenAiCredentialsForInternal,
  callOpenAIChat,
  isUserAllowed,
  denyMessage,
  publicConfig,
