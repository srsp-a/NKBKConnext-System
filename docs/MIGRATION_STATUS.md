...
| **CMS** | เว็บองค์กร WP → Firestore | migrate **440/440** เสร็จ · Hosting ทดสอบแล้ว · **ยังไม่ผูก `nkbkcoop.com`** |
...
| 4 | ผูก `nkbkcoop.com` | **ท้ายสุด** — เมื่อ CMS พร้อม (ดู `HOSTING_DOMAIN_CHECKLIST.md`) |
...
| CMS ทดสอบ | `https://admin-panel-nkbkcoop-cbf10.web.app/n/14499` | HTTP 200 |
...
| URL | `/news`, `/n/{id}` (rewrite → `post.html` ใน `firebase.json`) |
...
| โดเมนจริง | `nkbkcoop.com` ยังชี้ **WordPress** — สลับเมื่อ UI CMS พร้อม |
